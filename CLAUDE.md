...
- **Theme:** Windows 98 clinical workstation aesthetic (98.css) — cool gray palette with Arctic Blue accent (`#4F6EF7`), `surface-base: #0F1117`
...
- **Layout:** Menu bar + toolbar + sunken content panels + status bar. Left nav 180px. Desktop-only MVP (1024px+).
- **Components:** 98.css for Windows 98 UI chrome + 12 custom Svelte 5 health-domain components (AppShell, BiomarkerTable, AIClinicalNote, ImportDialog, etc.)
- **Font:** DM Sans (Google Fonts), type scale from 11px (micro) to 32px (display)
...
- **`lib/components/`** — 98.css primitives (`ui/`) + custom health-domain components (`health/`).
