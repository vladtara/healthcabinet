<script lang="ts">
	...
	function getTrendDirection(values: { value: number }[]): {
		label: string;
		symbol: string;
		colorClass: string;
	} {
		if (values.length < 2)
			return { label: 'Not enough data', symbol: '', colorClass: 'hc-trend-direction-stable' };
		...
		if (changePercent > 5)
			return { label: 'Increasing', symbol: '↑', colorClass: 'hc-trend-direction-up' };
		if (changePercent < -5)
			return { label: 'Decreasing', symbol: '↓', colorClass: 'hc-trend-direction-down' };
		return { label: 'Stable', symbol: '→', colorClass: 'hc-trend-direction-stable' };
	}
</script>
...
{#if trendQuery.isPending}
	<div class="animate-pulse" aria-hidden="true">
		...
		<div style="padding: 12px;">
			<div class="bg-muted h-32 w-full rounded"></div>
		</div>
	</div>
{:else if trendQuery.isError}
	<div>
		...
		<div style="padding: 12px; text-align: center;">
			<p class="text-muted-foreground text-xs">Unable to load trend for {canonicalName}.</p>
		</div>
	</div>
