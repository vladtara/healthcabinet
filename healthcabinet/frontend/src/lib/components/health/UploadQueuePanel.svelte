<script lang="ts">
	...
	import { countByStatus, getActiveEntry, type UploadQueueEntry } from '$lib/upload-queue';
	...
	let { queue, onActiveComplete, onActiveFailed, onActiveAuthError, onRetryEntry }: Props =
		$props();
...
<section aria-label={copy.queuePanelAria} style="display: flex; flex-direction: column; gap: 12px;">
	<header
		style="display: flex; justify-content: space-between; align-items: center; padding: 6px 10px; border-bottom: 1px solid var(--surface-raised, #2a2e38); font-size: 12px;"
	>
		<span style="font-weight: 600;">{copy.queueHeader}</span>
		<span aria-live="polite" style="color: var(--text-muted, #8a8f98);">
			{counts.completed}
			{copy.queueCompleteLabel} · {counts.partial}
			{copy.queuePartialLabel} · {counts.failed}
			{copy.queueFailedLabel} · {counts.queued + counts.uploading + counts.processing}
			{copy.queuePendingLabel}
		</span>
	</header>

	<ol role="list" style="list-style: none; padding: 0; margin: 0;">
		{#each queue as entry (entry.id)}
			<UploadQueueEntryRow {entry} isActive={activeEntry?.id === entry.id} onRetry={onRetryEntry} />
		{/each}
	</ol>
