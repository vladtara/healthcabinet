...
<section aria-label={copy.batchSummaryAria} style="display: flex; flex-direction: column; gap: 14px;">
	...
	{#if successfulEntries.length > 0}
		<div>
			<p style="margin: 0 0 6px; font-size: 12px; color: var(--text-muted, #8a8f98); text-transform: uppercase; letter-spacing: 0.04em;">
				{copy.batchSuccessfulHeader}
			</p>
			<ul role="list" style="list-style: none; padding: 0; margin: 0; border: 1px solid var(--surface-raised, #2a2e38);">
				{#each successfulEntries as entry (entry.id)}
					<li style="display: grid; grid-template-columns: auto 1fr auto auto; align-items: center; gap: 10px; padding: 6px 10px; border-bottom: 1px solid var(--surface-raised, #2a2e38);">
						<span aria-hidden="true">📄</span>
						<span title={entry.file.name} style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
							{entry.file.name}
						</span>
						<span
							class={entry.status === 'partial' ? 'hc-badge hc-badge-warning' : 'hc-pipeline-stage-done'}
						...
						{#if entry.documentId}
							<a href={`/documents/${entry.documentId}`} style="font-size: 12px; text-decoration: underline;">
		...
		<div>
			...
			<ul role="list" style="list-style: none; padding: 0; margin: 0; border: 1px solid var(--surface-raised, #2a2e38);">
				{#each failedEntries as entry (entry.id)}
					<li style="display: grid; grid-template-columns: auto 1fr auto; align-items: center; gap: 10px; padding: 6px 10px; border-bottom: 1px solid var(--surface-raised, #2a2e38);">
						<span aria-hidden="true">📄</span>
						<span title={entry.file.name} style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
							{entry.file.name}
						</span>
						...
						{#if entry.error}
							<span style="grid-column: 1 / -1; font-size: 11px; color: var(--status-action, #e05252);">
								{entry.error}
							</span>
						{/if}
					</li>
